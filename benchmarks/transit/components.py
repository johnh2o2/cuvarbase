...
def main():
    ...
    lc=tuple(np.array(d[f'{k}_{i}']) for k in ['t','y','dy']);b=Backend(cfg,d,len(lc[0]))
    if cfg.get('no_scatter'):
        b.bls._cached_conflict_scatter_perm=lambda n:None
    b.search([lc])
    times=[]
    for rep in range(3):
    ...
    for rep in range(3):
        ...
        f,q=m.transit_autofreq(grid_t,rho=1.,samples_per_peak=2,qmin_fac=.5,
            fmin=1/meta['pmax'],fmax=1/meta['pmin'])
        take=f<=1/meta['pmin'];f=f[take];q=q[take]
    ...
    fullbaseline=float(np.ptp(grid_t))
    ...
    dump(a.out/'summary.json',dict(status='ok',profile=meta['profile'],config=cfg,input_sha256=sha(a.input),
        ...
        grid_times_s=gridtimes,grid_median_s=float(np.median(gridtimes)),grid_baseline=fullbaseline,grid_observed_time_index=grid_index,n_auto_freqs=len(f),grid_float32_equal=bool(np.array_equal(f.astype(np.float32),d['freqs'].astype(np.float32))),
