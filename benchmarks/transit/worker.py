    ...
    def __init__(self,cfg,d,capacity):
        ...
        if k.startswith('v1_bls') or k.startswith('pypi_bls'):
            ...
            if k.startswith('pypi'):
                ...
                fn='full_bls_no_sol_optimized' if cfg.get('optimized') else 'full_bls_no_sol'
                self.functions=bls.compile_bls(function_names=[fn],use_optimized=cfg.get('optimized',False),block_size=cfg.get('block_size',256))
...
def main():
    ...
    record=dict(status='running',config=cfg,input_file=Path(a.input).name,input_sha256=sha(a.input),
        ...
        environment=dict(python=sys.version,packages={k:importlib.metadata.version(k) for k in ['numpy','scipy','pycuda','astropy','cupy-cuda12x','gputls','cuvarbase','periodfind'] if list(importlib.metadata.distributions(name=k))},
    ...
    try:
        ...
        try:
            warm=backend.search([lcs[ids[0]]]);backend.sync()
            record['first_api_candidates']=[o['candidate'] for o in warm]
            record['first_api_valid']=all(o['candidate'].get('api_result_valid',True) and o['candidate']['finite_fraction']==1. for o in warm)
            if a.timing and not record['first_api_valid']:raise RuntimeError('First timed API warmup returned an invalid result')
        except Exception:
            if a.timing:raise
        ...
        if a.timing:
            warm=backend.search([lcs[i] for i in ids]);times=[]
            record['batch_warmup_candidates']=[o['candidate'] for o in warm]
            if not all(o['candidate'].get('api_result_valid',True) and o['candidate']['finite_fraction']==1. for o in warm):raise RuntimeError('Batch timing warmup returned an invalid result')
            record['timed_candidates']=[]
            for rep in range(a.reps):
                backend.sync();start=time.perf_counter();outputs=backend.search([lcs[i] for i in ids]);backend.sync()
                times.append(time.perf_counter()-start)
                record['timed_candidates'].append([o['candidate'] for o in outputs])
                if not all(o['candidate'].get('api_result_valid',True) and o['candidate']['finite_fraction']==1. for o in outputs):raise RuntimeError('A timed repetition returned an invalid result')
        ...
        for indices in ([ids] if a.timing else [ids[j:j+chunksize] for j in range(0,len(ids),chunksize)]):
            if not a.timing:
                ...
                try:outputs=backend.search([lcs[i] for i in indices]);backend.sync()
                    ...
                    failure=traceback.format_exc();elapsed=time.perf_counter()-start
                    for i in indices:
                        truth=meta['cases'][i]
                        record['cases'].append(dict(index=i,injected=truth['injected'],snr=truth['target_white_oracle_snr'],
                            ...
                            api_result_valid=False,error=failure,search_s=elapsed/len(indices)))
                    dump(a.out/'summary.json',record);continue
                elapsed=time.perf_counter()-start
            else:elapsed=None
            if len(outputs)!=len(indices):raise RuntimeError('Output count does not match input count')
