...
"""
Provides an interface for analyzing light curves using the Multi-Harmonic
Fourier (MHF) periodogram.
"""
...
cdef extern from "./cuda/mhf.h":
    cdef cppclass CppMultiHarmonicFourier "MultiHarmonicFourier":
        CppMultiHarmonicFourier(size_t max_harmonics)

        void CalcMHFBatched(const vector[float*]& times,
...
cdef class MultiHarmonicFourier:
    """Multi-Harmonic Fourier periodogram light curve analysis.
    ...
    cdef CppMultiHarmonicFourier* mhf

    def __cinit__(self, max_harmonics=5):
        self.mhf = new CppMultiHarmonicFourier(max_harmonics)

    def __dealloc__(self):
        if self.mhf is not NULL:
            del self.mhf
    ...
    def calc(self,
        ...
        mhf_ndarr = np.zeros([len(times), n_per, n_pdt], dtype=np.float32)
        cdef float[:, :, ::1] mhf_view = mhf_ndarr

        self.mhf.CalcMHFBatched(
