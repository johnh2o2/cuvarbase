...
"""
Provides an interface for analyzing light curves using the Fast Phase-folding
Weighted (FPW) algorithm (Finkbeiner et al. 2025).
"""
...
cdef extern from "./cuda/fpw.h":
    cdef cppclass CppFPW "FPW":
        CppFPW(size_t num_bins)

        void CalcFPWBatched(const vector[float*]& times,
...
cdef class FPW:
    """Fast Phase-folding Weighted (FPW) light curve analysis.

    Attempts to determine the period of a light curve by phase-folding
    and computing a weighted chi-squared reduction statistic. Supports
    per-point uncertainties for inverse-variance weighting.
    ...
    def __cinit__(self, n_bins=10):
        self.fpw = new CppFPW(n_bins)

    def __dealloc__(self):
        if self.fpw is not NULL:
            del self.fpw
    ...
    def calc(self,
        ...
        fpw_ndarr = np.zeros([len(times), n_per, n_pdt], dtype=np.float32)
        cdef float[:, :, ::1] fpw_view = fpw_ndarr

        self.fpw.CalcFPWBatched(
