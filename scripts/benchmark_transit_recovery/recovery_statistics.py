...
def valid(case):
    # TLS explicitly represents trial periods with no admissible fit as NaN.
    # A finite native candidate from the remaining searched periods is still a
    # valid API result. Report partial spectra separately; never conceal them.
    return case.get('api_result_valid',True) and case.get('finite_fraction',0.)>0. and not case.get('error')


def score(case):
    v=case.get('score')
    return float(v) if v is not None and np.isfinite(v) and valid(case) else -np.inf
...
def summarize(calibration,heldout):
    ...
    period=np.array([c['recovered'] and valid(c) for c in injected],bool)
    detected_score=np.array([score(c)>threshold for c in injected])
    detection=period&detected_score
    alias=np.array([c.get('alias_recovered',False) and valid(c) for c in injected],bool)
    alias_detection=alias&detected_score
    ...
    for snr in [6.,8.,10.,14.]:
        ...
        bins.append(dict(snr=snr,n=n,period_recovered=kp,detected=k,alias_recovered=int(alias[take].sum()),alias_detected=int(alias_detection[take].sum()),recall=k/n,interval=wilson(k,n),
    ...
    return dict(threshold=threshold,n_injections=len(injected),n_calibration_nulls=len(calibration),n_heldout_nulls=len(nulls),
        period_recovered=int(period.sum()),detected=int(detection.sum()),alias_recovered=int(alias.sum()),alias_detected=int(alias_detection.sum()),false_positives=int(fp.sum()),
        ...
        invalid_calibration=sum(not valid(c) for c in calibration),
        invalid_heldout=sum(not valid(c) for c in heldout),
        partial_spectra_calibration=sum(0.<c.get('finite_fraction',0.)<1. for c in calibration),
        partial_spectra_heldout=sum(0.<c.get('finite_fraction',0.)<1. for c in heldout),
        masked_trials_calibration=sum(round(c.get('n_periods',0)*(1-c.get('finite_fraction',1.))) for c in calibration),
        masked_trials_heldout=sum(round(c.get('n_periods',0)*(1-c.get('finite_fraction',1.))) for c in heldout),
