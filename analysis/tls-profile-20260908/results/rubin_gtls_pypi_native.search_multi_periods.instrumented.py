def search_multi_periods(periods, t, y, dy, transit_depth_min, R_star_min, R_star_max, M_star_min, M_star_max, lc_arr, lc_cache_overview, T0_fit_margin, oversampling_factor, verbose, useLocalPTXCUBIN=False, GPUDeviceID=0, fast=False, legacy=False, SimplifyEdgeEffect=True, bar_location=0):
    with _TLS_PROFILE.segment('GTLS setup and allocations'):
        set_cuda_device(GPUDeviceID)
    with _TLS_PROFILE.segment('GTLS CUDA module lookup/compile'):
    ...
    for iterFlag in range(TotalIter):
        with _TLS_PROFILE.segment('GTLS duration-mask union'):
        ...
        with _TLS_PROFILE.segment('GTLS chunk allocations/transfers'):
            ...
            durationsGridGPU((gridSizeX, 1, 1), (blockSize,), (periodsGPU, durationsMaxGPU, durationsMinGPU, tLengthGPU, tSizeGPU, periodsSizeGPU))
        ...
        with _TLS_PROFILE.segment('GTLS folding/sorting'):
        ...
        with _TLS_PROFILE.segment('GTLS reorder/weights'):
            patchDataGPU = module.get_function('patchData')
            blockSize, gridSizeX = calcGridBlockSize(tSize + maxDuration)
            patchDataGPU((gridSizeX, singleCalcPeriods), (blockSize,), (patchedDatasGPU, patchedDysGPU, patchedDatasSizeGPU, sortIndexGPU, maxDurationGPU, yGPU, dyGPU, tSizeGPU))
            ...
            blockSize, gridSizeX = calcGridBlockSize(singleCalcPeriods)
            calcEdgeEffectCorrectionsGPU((gridSizeX, 1, 1), (blockSize, 1, 1), (edgeEffectCorrectionsGPU, patchedDatasGPU, inverseSquaredPatchedDysGPU, patchedDatasSizeGPU, maxDurationGPU, periodSizeGPU))
        with _TLS_PROFILE.segment('GTLS row-wise flux prefix sums'):
            ...
            calcAllFullSumGPU((gridSizeX, singleCalcPeriods, 1), (blockSize, 1, 1), (fullSumGPU, patchedDatasGPU, inverseSquaredPatchedDysGPU, patchedDatasSizeGPU, durationsGPU, durationsSizeGPU, periodSizeGPU))
            ...
            kernel_calc_error(grid=grid_dim_2d, block=block_dim_1d, args=(base_error, patchedDatasGPU, inverseSquaredPatchedDysGPU, patchedDatasSize, singleCalcPeriods))
            ...
            kernel_final_ootr(grid=grid_dim_3d, block=block_dim_3d, args=(ootrGPU, error_prefix_sum, fullSumGPU, durationsGPU, tSize, patchedDatasSize, len(singleDurations), singleCalcPeriods))
        with _TLS_PROFILE.segment('GTLS transit residual kernel'):
            calcAllLowestResidualsGPU = module.get_function('calcAllLowestResidualsGPUB_SignalTiled_v2')
            blockSize, gridSizeX = calcGridBlockSize(tSize)
            calcAllLowestResidualsGPU((gridSizeX, len(singleDurations), singleCalcPeriods), (blockSize, 1, 1), (lowestResidualsGPU, tSizeGPU, patchedDatasGPU, patchedDatasSizeGPU, durationsGPU, durationsSizeGPU, lcArrFullLengthGPU, lcArrMaxLenGPU, inverseSquaredPatchedDysGPU, overshootGPU, ootrGPU, fullSumGPU, edgeEffectCorrectionsGPU, datapointsGPU, cumsumGPU, transitDepthMinGPU))
        with _TLS_PROFILE.segment('GTLS reductions/chunk cleanup'):
            start_idx = iterFlag * singleCalcPeriods
            ...
            if verbose:
                pbar.update(1)
    with _TLS_PROFILE.segment('GTLS result/statistics processing'):
    ...
    with _TLS_PROFILE.segment('GTLS candidate refinement'):
    ...
    with _TLS_PROFILE.segment('GTLS result/statistics processing'):
        ...
        period = periods[periodIndex]
    with _TLS_PROFILE.segment('GTLS best-period fit/diagnostics'):
