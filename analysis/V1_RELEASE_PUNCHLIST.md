...
- [x] **Public API stubs raising NotImplementedError: ConditionalEntropyAsyncProcess.memory_requirement and LombScargleMemory.is_ready** — IMPLEMENTED: CE memory_requirement(n0, nf) returns a histogram-dominated byte estimate (parallel to the LS signature); LombScargleMemory.is_ready validates nf/lsp_g/data arrays and delegates to the NFFT memories (mirrors NFFTMemory.is_ready). Tests in TestApiStubsImplemented (is_ready test runs on the pod). Commit: HASHX
...
- [x] **Published LS survey throughput requires batch_size=1, but batched_run_const_nfreq defaults to batch_size=10 and no doc says so** — FIXED: default changed to batch_size=1 (the benchmarked-fastest configuration behind all published numbers) with docstring guidance; multi-stream overhead at batch_size>1 noted as undiagnosed. Test asserts the default. Commit: HASHX
