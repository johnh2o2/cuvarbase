...
- [x] **NUFFT-LRT: uniform grid spans only median(dt)*2N from t.min(), silently ignoring later data — wrong results for the advertised multi-season use case** — RESOLVED by the cut (see previous item); the grid-span defect is documented in the #36 close-out note and the CHANGELOG. Commit: PENDING_HASH
