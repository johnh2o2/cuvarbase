...
- [x] **eebls_transit sparse/standard discontinuity: sparse path ignores qmin_fac/qmax_fac/use_fast and searches all q in (0, 0.5]** — STANCE CONFIRMED (loudly document): the docstring warning block + runtime UserWarning + use_sparse=False escape hatch are the accepted v1.0 remedy; wiring q bounds into the sparse kernels stays a v1.1 item. Commit: 363a4d9
...
- [x] **Eager `import pycuda.autoprimaryctx` makes `import cuvarbase` require a working GPU, contradicting advertised CPU fallbacks** — DOCUMENTED (minimum release action): README no longer sells use_gpu=False/sparse_bls_cpu as a GPU-less mode; Prerequisites now state that import creates a CUDA context, pins device 0, and that CUDA_DEVICE/process-spawn are the multi-GPU options. Lazy context creation stays a v1.1 item. Commit: 363a4d9
...
- [x] **GPU Lomb-Scargle supports only 1 harmonic ('right now' admission) despite CPU multiharmonic helpers in the same file** — FORMALLY DEFERRED: the feature gap stays (README lists multiharmonic GLS under Planned Features, honest); the bare Exception is now NotImplementedError (error-hygiene pass). No GPU work this release. Commit: d458b7f
...
- [x] **eebls_gpu_fast has no noverlap parameter ('yet') — phase-binning bias when optimal q is near qmin** — FORMALLY DOCUMENTED: docstring already states the limitation and the dphi re-run workaround; kept as a documented limit (no code change this release). Commit: 363a4d9
...
- [x] **NFFT filter radius m chosen by a heuristic the code itself flags as wrong** — DOCUMENTED: estimate_m docstring now carries a user-facing warning (heuristic bound, pass m explicitly for guaranteed accuracy); implementing the L1-norm bound stays a v1.1 item. Commit: 363a4d9
...
- [x] **Public API stubs raising NotImplementedError: ConditionalEntropyAsyncProcess.memory_requirement and LombScargleMemory.is_ready** — IMPLEMENTED: CE memory_requirement(n0, nf) returns a histogram-dominated byte estimate (parallel to the LS signature); LombScargleMemory.is_ready validates nf/lsp_g/data arrays and delegates to the NFFT memories (mirrors NFFTMemory.is_ready). Tests in TestApiStubsImplemented (is_ready test runs on the pod). Commit: 62e2463
...
- [x] **Fast CE kernels incompatible with weighted CE; mag_overlap incompatible with balanced_magbins (loud guards over real feature gaps)** — FORMALLY CUT per CE maintenance mode: unsupported combinations documented in ce.rst (with the periodfind referral); guards now raise ValueError (error-hygiene pass d458b7f). Commit: 363a4d9
...
- [x] **#33 PDM follow-ups publicly promised in our own issue comment: batched multi-LC API, memory-capped large_run, perf benchmark — all absent** — RE-SCOPED on the issue: validation checkbox confirmed done (gate record linked); the three feature items explicitly moved to v1.1 in a new comment. Commit: e5b808e
...
- [x] **FFA-BLS: formally cut (negative result, ~14x slower); only residual decision is whether the spec doc ships** — DECIDED: docs/FBLS_GPU_SPEC.md stays in the repo as the negative-result record (its STATUS banner is unambiguous); nothing user-facing advertises FFA, no sdist/docs-build inclusion change needed. Commit: 363a4d9
...
- [x] **eebls_gpu_batch large-ndata regression: ~12x slower than the single-LC loop for TESS-like data — undiagnosed, unguarded, and contradicted by its own docs** — DOCUMENTED+GUARDED (minimum v1.0 action): docstring warning block with the measured numbers; runtime UserWarning above ndata=10,000; BENCHMARK_RESULTS prose no longer says 'as fast or faster'. nsys diagnosis stays queued (v1.1 candidate). Commit: 62e2463
...
- [x] **Page-locked (pinned) host buffers never restored — allocate_pinned_arrays is a misnomer and async transfers silently serialize** — RENAMED+DOCUMENTED: BLSMemory.allocate_host_arrays (deprecated warning on the old name); 'pinned' wording corrected in all memory classes' allocate_pinned_cpu docstrings and the bls_memory module docstring (page-aligned, NOT page-locked; async transfers stage synchronously). Restoring true pinned buffers stays a v1.1 perf item. Commit: 62e2463
...
- [x] **Adaptive block-size heuristic considers only ndata, not nbins** — DOCUMENTED: _choose_block_size docstring states the ndata-only domain, the ignored nbins/occupancy effects, and the benchmark regime behind the 1.4-5.3x claim; extending the heuristic stays a v1.1 item. Commit: 363a4d9
...
- [x] **Published LS survey throughput requires batch_size=1, but batched_run_const_nfreq defaults to batch_size=10 and no doc says so** — FIXED: default changed to batch_size=1 (the benchmarked-fastest configuration behind all published numbers) with docstring guidance; multi-stream overhead at batch_size>1 noted as undiagnosed. Test asserts the default. Commit: 62e2463
...
- [x] **scikit-cuda remains the cuFFT backend for LS/NFFT; full replacement publicly promised 'post-1.0' (issue #63)** — DEFERRED per standing decision: CHANGELOG 'Known limitations' release note added (shim + lazy imports shipped; replacement committed post-1.0) and a confirmation comment posted on #63. Commit: e5b808e
...
- [x] **bls_optimized.cu remains a near-duplicate of bls.cu — kernel-drift hazard with a prior shipped-bug precedent** — GUARDED: test_kernel_drift.py extracts every shared-name device/global function from both files and fails CI if they differ beyond normalized cosmetics (comments, whitespace, float-literal suffixes, mod1/mod1_fast); reduction_max is whitelisted as intentionally divergent (full tree vs tree+shuffle, both verified correct). Guard verified to catch injected drift. Full templating merge stays a v1.1 item. Commit: ceae686
...
- [x] **Error-handling hygiene: assert-based validation vanishes under python -O; user input errors raised as bare Exception** — FIXED: all ~40 assert sites across lombscargle/ce/bls + memory classes converted to typed raises with messages (ValueError for input, RuntimeError for state, NotImplementedError for the 1-harmonic gap); all 11 bare `raise Exception` sites retyped. test_error_hygiene.py greps the runtime modules for both patterns (fails pre-fix) and runs check_k0 under `python -O` in a subprocess. Commit: d458b7f
...
- [x] **Bus-factor/release-ritual items pending: JOSS paper, ASCL record update, co-maintainer onboarding** — DISPOSITIONED: USER ACTIONS — JOSS submission, ASCL record update, and inviting @astrobatty as co-maintainer cannot be done by automation and are not release blockers; flagged in the release-candidate report for maintainer follow-up. Commit: e5b808e
...
- [x] **v1.0.0 tag not merged to master; PyPI publish deferred; local master stale behind origin** — BLOCKED BY DESIGN: merging master, moving the tag, and publishing are the release closing moves and on the NEVER-without-explicit-go list; tracked in the release-candidate report. Commit: e5b808e
...
- [x] **Sphinx autodoc covers no post-0.2.6 modules** — FIXED: cuvarbase.rst now has automodule entries for bls_frequencies, cufinufft_backend, tls/tls_grids/tls_models/tls_stats (marked experimental), the four memory modules, and base.async_process. Clean-environment Sphinx render check remains a release-day step (no sphinx locally). Commit: ceae686
...
- [x] **Benchmark claim traceability: adaptive-kernel numbers (1.4-5.3x; 90x for ndata<64) and the '~1 second' fBLS comparison have no raw data in the repo** — HALF FIXED: the fBLS sentence now cites Shahaf et al. table 1 and the repo's own 0.17 s/LC Kepler measurement (JSON path included); the adaptive-vs-fixed JSON rerun is in the GPU queue (pod batch commits the output). Commit: e5b808e
...
- [x] **GitHub tracker reconciliation at release: #33 checkbox (verified done), #15 (verified done, close after docs rebuild), #19 (verified: tables but no plots), #32 (close-as-wontfix per CE descope), #28 (status comment)** — PARTIALLY EXECUTED + RELEASE-DAY CHECKLIST: #33 re-scoped (comment posted); #14 fix noted for closure; the remaining closes (#15 after docs rebuild, #19 close-with-tables, #32 wontfix, #28 status comment, #36 note already posted) are listed in the release-candidate report as the release-day pass tied to the final tag. Commit: e5b808e
...
- [x] **#17: BLS/LS power-spectrum convention — docstrings now state 1 - chi2/chi2_0, but bls.rst has no convention discussion and selectable conventions were never implemented** — DOCUMENTED: 'Power-spectrum convention' sections added to bls.rst (vs astropy BoxLeastSquares objectives, phi0 epoch convention, pointer to #17 for selectable conventions) and lomb.rst (standard normalization, ZK2009 floating mean, astropy comparability). Selectable conventions explicitly deferred on the issue at release. Commit: ceae686
...
- [x] **#29: Documentation refactor umbrella (docstring audit, example notebooks) incomplete and un-scoped** — SCOPED on the issue: comment posted listing what v1.0 delivered and deferring the docstring audit + notebooks to v1.1. Commit: e5b808e
...
- [x] **#30: Code-convention standardization + CONTRIBUTING guidelines untouched** — RESOLVED: CONTRIBUTING.md already ships (Python-version line corrected to 3.9+); renaming deferred to a major cycle with deprecation aliases — comment posted on the issue. Commit: e5b808e
