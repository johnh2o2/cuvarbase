...
- [ ] **lomb_scargle_async direct-sums branch gates host transfer on the wrong flag (transfer_to_device instead of transfer_to_host)**
...
- [ ] **use_cufinufft=True silently ignored by module-level lomb_scargle_async when cufinufft is not installed**
...
- [ ] **NUFFT-LRT: all computation on CPU — 6 compiled CUDA kernels never invoked (GPU+nvcc required for nothing); README body still says 'GPU Accelerated'**
...
- [ ] **NUFFT-LRT: uniform grid spans only median(dt)*2N from t.min(), silently ignoring later data — wrong results for the advertised multi-season use case**
