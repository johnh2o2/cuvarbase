...
- [x] **eebls_transit sparse/standard discontinuity: sparse path ignores qmin_fac/qmax_fac/use_fast and searches all q in (0, 0.5]** — STANCE CONFIRMED (loudly document): the docstring warning block + runtime UserWarning + use_sparse=False escape hatch are the accepted v1.0 remedy; wiring q bounds into the sparse kernels stays a v1.1 item. Commit: HASHX
...
- [x] **eebls_gpu_fast has no noverlap parameter ('yet') — phase-binning bias when optimal q is near qmin** — FORMALLY DOCUMENTED: docstring already states the limitation and the dphi re-run workaround; kept as a documented limit (no code change this release). Commit: HASHX
...
- [x] **NFFT filter radius m chosen by a heuristic the code itself flags as wrong** — DOCUMENTED: estimate_m docstring now carries a user-facing warning (heuristic bound, pass m explicitly for guaranteed accuracy); implementing the L1-norm bound stays a v1.1 item. Commit: HASHX
...
- [x] **Fast CE kernels incompatible with weighted CE; mag_overlap incompatible with balanced_magbins (loud guards over real feature gaps)** — FORMALLY CUT per CE maintenance mode: unsupported combinations documented in ce.rst (with the periodfind referral); guards now raise ValueError (error-hygiene pass d458b7f). Commit: HASHX
...
- [x] **FFA-BLS: formally cut (negative result, ~14x slower); only residual decision is whether the spec doc ships** — DECIDED: docs/FBLS_GPU_SPEC.md stays in the repo as the negative-result record (its STATUS banner is unambiguous); nothing user-facing advertises FFA, no sdist/docs-build inclusion change needed. Commit: HASHX
...
- [x] **Adaptive block-size heuristic considers only ndata, not nbins** — DOCUMENTED: _choose_block_size docstring states the ndata-only domain, the ignored nbins/occupancy effects, and the benchmark regime behind the 1.4-5.3x claim; extending the heuristic stays a v1.1 item. Commit: HASHX
