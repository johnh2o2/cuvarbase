...
NEW = r'''// Step 7 (PATCHED): incremental pair decode -- O(N + pairs/blockDim) per thread instead of O(N * pairs/blockDim)
        ...
        {
            // keep the stock thread<->pair assignment for the wrapped section (p = total_nonwrap + idx, p == tid mod blockDim)
            unsigned int r = total_nonwrap % blockDim.x;
            unsigned int p0 = (tid >= r) ? tid - r : tid + blockDim.x - r;
            unsigned int i = 1, idx = p0;
            while (i < N && idx >= i) { idx -= i; i++; }
            for (unsigned int p = p0; p < total_wrap; p += blockDim.x) {
                unsigned int k = idx;
                float phi0 = sh_phi[i];
...
def run(kern, t, y, dy, freqs, qmn, qmx, bs, ndata):
    ...
    launch(); allt = [launch() for _ in range(7)]; m = float(np.min(allt))/1e3
...
for ndata in (150, 300, 499):
    ...
    for bounds_lab, (qa, qb) in (('keplerian', (qmins, qmaxs)), ('unbounded', (np.zeros_like(qmins), np.full_like(qmaxs, 0.5)))):
        for bs in (64, 128, 256):
            ...
            out[key] = dict(stock_ms_min=mo*1e3, patched_ms_min=mn*1e3, speedup=mo/mn, q_phi_identical=bool(np.array_equal(qo, qn) and np.array_equal(pho, phn)),
...
for lab, kk in (('patched', k_new), ('stock', k_old)):
    mo, po, qo, pho, _ = run(kk, t, y, dy, sub, qmins[::200], qmaxs[::200], 64, 150)
    out['cpu_vs_%s_gpu' % lab] = dict(max_abs_diff=float(np.max(np.abs(pc - po))), corr=float(np.corrcoef(pc, po)[0,1]), argmax_same=bool(np.argmax(pc)==np.argmax(po)))
    print(lab, out['cpu_vs_%s_gpu' % lab])
json.dump(out, open('/workspace/scratch/sparse_proto2.json','w'), indent=1)
