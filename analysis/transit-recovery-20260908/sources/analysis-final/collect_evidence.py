...
def main():
    ...
    if a.node=='original':
        # Permit local arithmetic/recovery review while the much larger full
        # periodogram archive is transferring. Final publication still requires
        # independent verification of all full arrays and transferred files.
        with tarfile.open(BASE/'recovery-original-summaries.tar.gz','w:gz') as t:
            small=list(r.glob('*.json'))+list((r/'results').glob('*/summary.json'))+list((r/'results').glob('*/execution.json'))
            for p in sorted(small):t.add(p,arcname=str(p.relative_to(r)))
