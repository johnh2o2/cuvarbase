...
def find_nearest_indices(target_array, search_array):
    """Find nearest indices of target values in search array."""
...
def _setup_gpu_environment(GPUDeviceID, T0_fit_margin=None, compile_module=True):
    """Setup GPU environment and compile CUDA code.
    
    Args:
        GPUDeviceID: GPU device ID
        T0_fit_margin: T0 fitting margin (None for single period search)
        compile_module: Whether to compile the module (default True)
    """
    set_cuda_device(GPUDeviceID)
    
    gpu_code = GPUFun.getGPUCode()
    
    # Apply T0 fit margin modification if specified
    if T0_fit_margin is not None:
    ...
    module = cp.RawModule(code=gpu_code,options=('-lineinfo',))
    if compile_module:
        module.compile()
    return module

def _prepare_light_curve_arrays(single_durations, single_lc_arr, single_lc_cache_overview):
    """Prepare light curve arrays for GPU processing."""
    ...
    return lc_arr_max_len_gpu, lc_arr_full_length_gpu, overshoot_gpu

def _create_gpu_computation_arrays(periods_count, t_size, max_duration, durations_count):
    """Create common GPU arrays used in computation."""
    
    # Core computation arrays
    patched_datas_gpu = cp.empty((periods_count, t_size + max_duration), dtype=cp.float32)
    patched_dys_gpu = cp.empty((periods_count, t_size + max_duration), dtype=cp.float32)
    inverse_squared_patched_dys_gpu = cp.empty((periods_count, t_size + max_duration), dtype=cp.float32)
    edge_effect_corrections_gpu = cp.empty(periods_count, dtype=cp.float32)
    
    # Processing arrays
    full_sum_gpu = cp.empty((periods_count, durations_count), dtype=cp.float32)
    cumsum_gpu = cp.empty((periods_count, t_size + max_duration), dtype=cp.float32)
    ootr_gpu = cp.empty((periods_count, durations_count, t_size), dtype=cp.float32)
    
    return {
        'patched_datas_gpu': patched_datas_gpu,
        'patched_dys_gpu': patched_dys_gpu,
        'inverse_squared_patched_dys_gpu': inverse_squared_patched_dys_gpu,
        'edge_effect_corrections_gpu': edge_effect_corrections_gpu,
        'full_sum_gpu': full_sum_gpu,
        'cumsum_gpu': cumsum_gpu,
        'ootr_gpu': ootr_gpu
    }

def _execute_common_gpu_kernels_phase1(module, t_gpu, periods_gpu, phases_gpu, sort_index_gpu,
                                       periods_size_gpu, t_size_gpu, periods_count, t_size):
    """Execute phase 1 GPU kernels: phase folding and sorting."""
    # 1. Phase folding
    fast_fold_gpu = module.get_function('foldFast')
    block_size, grid_size_x = calcGridBlockSize(t_size)
    fast_fold_gpu((grid_size_x, periods_count), (block_size,), 
                 (t_gpu, periods_gpu, phases_gpu, periods_size_gpu, t_size_gpu))
    
    # 2. Sorting (optimized chunking)
    for i in range(1, MAX_ARGSORT_ITERATIONS + 1):
        start_idx = (i - 1) * periods_count // MAX_ARGSORT_ITERATIONS
        end_idx = i * periods_count // MAX_ARGSORT_ITERATIONS
        sort_index_gpu[start_idx:end_idx] = phases_gpu[start_idx:end_idx].argsort()

def _execute_common_gpu_kernels_phase2(module, patched_datas_gpu, patched_dys_gpu, 
                                       patched_data_size_gpu, sort_index_gpu, max_duration_gpu,
                                       y_gpu, dy_gpu, t_size_gpu, inverse_squared_patched_dys_gpu,
                                       edge_effect_corrections_gpu, period_size_gpu,
                                       periods_count, t_size, max_duration, patched_data_size):
    """Execute phase 2 GPU kernels: data patching and inverse squared dy calculation."""
    # 3. Patch data
    patch_data_gpu = module.get_function('patchData')
    block_size, grid_size_x = calcGridBlockSize(t_size + max_duration)
    patch_data_gpu((grid_size_x, periods_count), (block_size,),
                  (patched_datas_gpu, patched_dys_gpu, patched_data_size_gpu,
                   sort_index_gpu, max_duration_gpu, y_gpu, dy_gpu, t_size_gpu))
    ...
    calc_inverse_squared_patched_dy_gpu((grid_size_x, periods_count, 1), (block_size, 1, 1),
                                       (inverse_squared_patched_dys_gpu, patched_dys_gpu, 
                                        patched_data_size_gpu))
    
    # 5. Calculate edge effect corrections
    calc_edge_effect_corrections_gpu = module.get_function('calcEdgeEffectCorrections')
    block_size, grid_size_x = calcGridBlockSize(periods_count)
    calc_edge_effect_corrections_gpu((grid_size_x, 1, 1), (block_size, 1, 1),
                                    (edge_effect_corrections_gpu, patched_datas_gpu, 
                                     inverse_squared_patched_dys_gpu, patched_data_size_gpu,
                                     max_duration_gpu, period_size_gpu))

def _execute_common_gpu_kernels_phase3(module, cumsum_gpu, full_sum_gpu, ootr_gpu,
                                       patched_datas_gpu, inverse_squared_patched_dys_gpu,
                                       patched_data_size_gpu, durations_gpu, durations_size_gpu,
                                       period_size_gpu, t_size_gpu, periods_count, t_size, 
                                       single_durations):
    """Execute phase 3 GPU kernels: cumulative calculations and out-of-transit residuals."""
    ...
    calc_all_full_sum_gpu((grid_size_x, periods_count, 1), (block_size, 1, 1),
                         (full_sum_gpu, patched_datas_gpu, inverse_squared_patched_dys_gpu,
                          patched_data_size_gpu, durations_gpu, durations_size_gpu, period_size_gpu))
    ...
    block_size, grid_size_x = calcGridBlockSize(t_size)
    calc_ootr_step1_gpu((grid_size_x, len(single_durations), periods_count), (block_size, 1, 1),
                       (ootr_gpu, patched_datas_gpu, durations_gpu, durations_size_gpu,
                        inverse_squared_patched_dys_gpu, patched_data_size_gpu, t_size_gpu))
    ...
    calc_ootr_step2_gpu((grid_size_x, len(single_durations), periods_count), (block_size, 1, 1),
                       (ootr_gpu, durations_size_gpu, patched_data_size_gpu,
                        durations_gpu, t_size_gpu, full_sum_gpu))
    
    return ootr_gpu

def _execute_final_residuals_kernel(module, kernel_name, lowest_residuals_gpu, t_size_gpu,
                                   patched_datas_gpu, patched_data_size_gpu, durations_gpu,
                                   durations_size_gpu, lc_arr_full_length_gpu, lc_arr_max_len_gpu,
                                   inverse_squared_patched_dys_gpu, overshoot_gpu, ootr_gpu,
                                   full_sum_gpu, edge_effect_corrections_gpu, datapoints_gpu,
                                   cumsum_gpu, transit_depth_min_gpu, periods_count, t_size,
                                   single_durations, durations_max_gpu=None, durations_min_gpu=None):
    """Execute final residuals calculation kernel with configurable kernel name."""
    calc_lowest_residuals_gpu = module.get_function(kernel_name)
    block_size, grid_size_x = calcGridBlockSize(t_size)
    
    if kernel_name == 'calcAllLowestResidualsGPUBNoSkip':
        # Single period search kernel with additional duration parameters
        calc_lowest_residuals_gpu((grid_size_x, len(single_durations), periods_count), (block_size, 1, 1),
                                 (lowest_residuals_gpu, t_size_gpu, patched_datas_gpu,
                                  patched_data_size_gpu, durations_gpu, durations_size_gpu,
...
def _initialize_gpu_arrays(periods, durations, single_calc_periods, t_size, patched_data_size, for_refinement=False):
    """Initialize all GPU arrays and constants.
    
    Args:
        periods: Array of periods
        durations: Array of durations  
        single_calc_periods: Number of periods per batch
        t_size: Size of time array
        patched_data_size: Size of patched data
        for_refinement: Whether this is for refinement search (affects return values)
    """
    # Result arrays
    ...
    if for_refinement:
        # For refinement search, return simplified arrays
        return location_gpu, lowest_residuals_each_period_gpu, arrays_dict
    else:
        # For main search, include additional period-related arrays
        ...
        # Add iteration flag for main search
        arrays_dict['iter_flag_gpu'] = cp.int32(0)
...
def _setup_duration_grids(module, periods, t, arrays_dict, full_durations_max_gpu, 
                         full_durations_min_gpu, durations, for_refinement=False):
    ...
    Args:
        ...
        periods: Array of periods
        t: Time array
        arrays_dict: Dictionary of GPU arrays
        full_durations_max_gpu: GPU array for maximum durations (None for refinement)
        full_durations_min_gpu: GPU array for minimum durations (None for refinement)
        durations: Array of durations
        for_refinement: Whether this is for refinement search
    """
    ...
    if for_refinement:
        # For refinement, create new duration arrays
        full_durations_max_gpu = cp.empty((len(periods),), dtype=cp.int32)
        full_durations_min_gpu = cp.empty((len(periods),), dtype=cp.int32)
...
def _setup_current_duration_grid(duration_bool_array_gpu, durations_grid_collection_gpu, 
                                iter_flag, total_iterations, single_calc_periods, is_refinement=False):
    """Setup duration grid for current iteration.
    
    Args:
        duration_bool_array_gpu: Boolean array for duration filtering
        durations_grid_collection_gpu: Collection of duration grids
        iter_flag: Current iteration flag
        total_iterations: Total number of iterations
        single_calc_periods: Number of periods per batch
        is_refinement: Whether this is for refinement search
    """
    if iter_flag == total_iterations - 1:
        start_idx = iter_flag * single_calc_periods
...
def _execute_gpu_batch_computation(module, current_periods, t, y_gpu, dy_gpu, 
                                  ...
                                  arrays_dict, transit_depth_min, kernel_name='calcAllLowestResidualsGPUB'):
    """Execute GPU computations for current batch using unified array creation.
    
    Args:
        module: Compiled CUDA module
        current_periods: Array of periods to process
        t, y_gpu, dy_gpu: Time series data
        single_durations, single_lc_arr, single_lc_cache_overview: Duration and light curve data
        single_calc_periods: Number of periods in batch
        t_size, max_duration, patched_data_size: Size parameters
        arrays_dict: Dictionary of common GPU arrays
        transit_depth_min: Minimum transit depth
        kernel_name: Name of the final residuals kernel to use
    """
    ...
    # Use unified array creation
    gpu_arrays = _create_gpu_computation_arrays(single_calc_periods, t_size, max_duration, len(single_durations))
    
    # Light curve processing using helper function
    lc_arr_max_len_gpu, lc_arr_full_length_gpu, overshoot_gpu = _prepare_light_curve_arrays(
        single_durations, single_lc_arr, single_lc_cache_overview)
    ...
    # Execute GPU kernels with specified kernel name
    _execute_gpu_kernels(module, t_gpu, periods_gpu, phases_gpu, sort_index_gpu,
                        periods_size_gpu, arrays_dict, gpu_arrays['patched_datas_gpu'], 
                        gpu_arrays['patched_dys_gpu'], y_gpu, dy_gpu, max_duration_gpu, 
                        gpu_arrays['inverse_squared_patched_dys_gpu'],
                        gpu_arrays['edge_effect_corrections_gpu'], period_size_gpu, 
                        gpu_arrays['cumsum_gpu'], gpu_arrays['full_sum_gpu'], durations_gpu, 
                        durations_size_gpu, gpu_arrays['ootr_gpu'], lowest_residuals_gpu, 
                        lc_arr_full_length_gpu, lc_arr_max_len_gpu, overshoot_gpu,
                        datapoints_gpu, transit_depth_min_gpu, single_calc_periods, t_size, 
                        max_duration, patched_data_size, single_durations, kernel_name)
...
def _execute_gpu_kernels(module, t_gpu, periods_gpu, phases_gpu, sort_index_gpu,
                        ...
                        single_calc_periods, t_size, max_duration, patched_data_size,
                        single_durations, kernel_name='calcAllLowestResidualsGPUB'):
    """Execute all GPU kernels in sequence using common functions.
    
    Args:
        kernel_name: Name of the final residuals kernel to use (default: 'calcAllLowestResidualsGPUB')
    """
    
    # Phase 1: Phase folding and sorting
    _execute_common_gpu_kernels_phase1(module, t_gpu, periods_gpu, phases_gpu, sort_index_gpu,
                                       periods_size_gpu, arrays_dict['t_size_gpu'], 
                                       single_calc_periods, t_size)
    
    # Phase 2: Data patching and inverse squared dy calculation
    _execute_common_gpu_kernels_phase2(module, patched_datas_gpu, patched_dys_gpu,
                                       arrays_dict['patched_data_size_gpu'], sort_index_gpu, 
                                       max_duration_gpu, y_gpu, dy_gpu, arrays_dict['t_size_gpu'],
                                       inverse_squared_patched_dys_gpu, edge_effect_corrections_gpu,
                                       period_size_gpu, single_calc_periods, t_size, max_duration, 
                                       patched_data_size)
    
    # Phase 3: Cumulative calculations and OOTR
    ootr_gpu = _execute_common_gpu_kernels_phase3(module, cumsum_gpu, full_sum_gpu, ootr_gpu,
                                                  patched_datas_gpu, inverse_squared_patched_dys_gpu,
                                                  arrays_dict['patched_data_size_gpu'], durations_gpu,
                                                  durations_size_gpu, period_size_gpu, 
                                                  arrays_dict['t_size_gpu'], single_calc_periods,
                                                  t_size, single_durations)
    
    # Phase 4: Final residuals calculation with configurable kernel
    _execute_final_residuals_kernel(module, kernel_name, lowest_residuals_gpu,
                                   arrays_dict['t_size_gpu'], patched_datas_gpu, 
                                   arrays_dict['patched_data_size_gpu'], durations_gpu,
                                   durations_size_gpu, lc_arr_full_length_gpu, lc_arr_max_len_gpu,
                                   inverse_squared_patched_dys_gpu, overshoot_gpu, ootr_gpu,
                                   full_sum_gpu, edge_effect_corrections_gpu, datapoints_gpu,
                                   cumsum_gpu, transit_depth_min_gpu, single_calc_periods, 
                                   t_size, single_durations)
...
def search_multi_periods_again(
    ...
    module = _setup_gpu_environment(GPUDeviceID, T0_fit_margin=None, compile_module=False)
    ...
    (location_gpu, lowest_residuals_each_period_gpu, 
     arrays_dict) = _initialize_gpu_arrays(periods, durations, single_calc_periods, 
                                          t_size, patched_data_size, for_refinement=True)
    
    # Step 5: Setup duration grids for all periods
    duration_bool_array_gpu = _setup_duration_grids(
        module, periods, t, arrays_dict, None, None, durations, for_refinement=True)
    ...
    for iter_flag in range(total_iterations):
        ...
        durations_grid_current = _setup_current_duration_grid(
            duration_bool_array_gpu, durations_grid_collection_gpu, iter_flag, 
            total_iterations, single_calc_periods, is_refinement=True)
        ...
        batch_results = _execute_gpu_batch_computation(
            ...
            patched_data_size, arrays_dict, transit_depth_min, 
            kernel_name='calcAllLowestResidualsGPUBNoSkipTemp')
...
def _execute_single_period_gpu_kernels(module, t_gpu, periods_gpu, phases_gpu, sort_index_gpu,
                                      ...
                                      durations_max_gpu, durations_min_gpu, t_size, max_duration, 
                                      single_durations):
    """Execute all GPU kernels for single period search using common functions."""
    
    # Phase 2: Data patching and inverse squared dy calculation (Phase 1 already done in computation function)
    _execute_common_gpu_kernels_phase2(module, patched_datas_gpu, patched_dys_gpu,
                                       arrays_dict['patched_data_size_gpu'], sort_index_gpu, 
                                       arrays_dict['max_duration_gpu'], y_gpu, dy_gpu, arrays_dict['t_size_gpu'],
                                       inverse_squared_patched_dys_gpu, edge_effect_corrections_gpu,
                                       arrays_dict['period_size_gpu'], 1, t_size, 
                                       max_duration, arrays_dict['patched_data_size_gpu'].get()[0])
    
    # Phase 3: Cumulative calculations and OOTR
    ootr_gpu = _execute_common_gpu_kernels_phase3(module, cumsum_gpu, full_sum_gpu, ootr_gpu,
                                                  patched_datas_gpu, inverse_squared_patched_dys_gpu,
                                                  arrays_dict['patched_data_size_gpu'], durations_gpu,
                                                  arrays_dict['durations_size_gpu'], arrays_dict['period_size_gpu'], 
                                                  arrays_dict['t_size_gpu'], 1,
                                                  t_size, single_durations)
    
    # Phase 4: Final residuals calculation using NoSkip kernel
