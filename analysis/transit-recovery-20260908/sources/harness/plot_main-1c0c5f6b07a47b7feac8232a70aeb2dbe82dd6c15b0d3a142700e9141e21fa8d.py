...
COLORS={'bls_v1':'#008566','bls_v1_batch':'#008566','tls_v1':'#008566','bls_pypi':'#2466aa','bls_cpu':'#c66a17','bls_gpu':'#8957a5','gtls':'#8957a5'}
...
def main():
    ...
    gs=fig.add_gridspec(4,3,left=.115,right=.978,top=.86,bottom=.125,hspace=.54,wspace=.60,height_ratios=[1.15,1.,.85,1.])
    fig.text(.045,.966,'How much faster is cuvarbase—and what does it recover?',fontsize=23,weight='bold',color='#172a3a')
    fig.text(.045,.936,'Measured search time, with sensitivity checked on independent transit injections',fontsize=14,color='#526270')
    fig.legend(handles=[Line2D([],[],marker='o',color='#334a5e',markerfacecolor='white',linestyle='none',label='One source'),
    ...
    for col,p in enumerate(PROFILES):
        ...
        for family,trow,rrow,methods,v1 in [('BLS',0,1,['bls_v1','bls_pypi','bls_cpu','bls_gpu'],'bls_v1'),('TLS',2,3,['tls_v1','gtls'],'tls_v1')]:
            ...
            for index,m in enumerate(methods):
                ...
                qualification=cc.get((p,ref,m));mark='†' if qualification and qualification['comparable_detection'] else ''
                text=time_label(values[1]) if m==v1 else f"{values[1]/tt[p,v1,'batch16']['seconds_per_source']:.1f}×{mark}"
                ax.annotate(text,(max(values),index),xytext=(7,0),textcoords='offset points',va='center',fontsize=10,color=COLORS[m],weight='bold' if m==v1 else 'normal')
            ax.set_xscale('log');ax.set_xlim(min(xs)/1.7,max(xs)*5.0);ax.set_ylim(len(methods)-.5,-.7)
            ...
            draw=[]
            for m in methods:
                tag='bls_v1_batch' if m=='bls_v1' else m
            ...
            if family=='BLS' and rr[p,'bls_v1']['detected_vector']!=rr[p,'bls_v1_batch']['detected_vector']:
                d=rr[p,'bls_v1'];rx.plot([q['snr'] for q in d['by_snr']],[q['recall']*100 for q in d['by_snr']],
                    '--',color=COLORS[v1],lw=1,alpha=.75,label='v1 single')
            ...
            fpr=[rr[p,('bls_v1_batch' if m=='bls_v1' else m)]['false_positive_rate']*100 for m in methods]
            rx.set_title(f'Held-out recovery · observed null FPR {min(fpr):.1f}–{max(fpr):.1f}%',loc='left',fontsize=10,pad=9,color='#526270')
    fig.text(.045,.085,'Speed labels compare batch throughput with v1. †: paired tests support <5 percentage-point loss in detection recovery and <5-point FPR increase.',fontsize=10,color='#394d5d')
    fig.text(.045,.065,'Unmarked ratios are timing comparisons with sensitivity differences or insufficient evidence of a match. Error bars: 95% Wilson intervals; 32 injections / SNR / survey.',fontsize=10,color='#394d5d')
