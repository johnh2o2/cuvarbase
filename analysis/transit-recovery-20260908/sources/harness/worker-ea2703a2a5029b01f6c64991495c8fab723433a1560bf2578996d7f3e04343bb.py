...
class Backend:
    ...
    def search(self,lcs):
        ...
        if k.startswith('v1_bls') or k.startswith('pypi_bls'):
            kw=dict(qmin=.5*self.q,qmax=2*self.q,dlogq=.1,noverlap=c.get('noverlap',3),ignore_negative_delta_sols=True)
            if k=='v1_bls_batch':
                powers=self.bls.eebls_gpu_batch(lcs,self.f,**kw)
            ...
            for lc in lcs:
                kw=dict(periods=self.tp,R_star=1,M_star=1,oversampling_factor=3,T0_fit_margin=c.get('margin',.125),
                    duration_grid_step=1.1,verbose=False,show_progress_bar=False,transit_template='default')
                if c.get('density'):
                    kw.update(R_star_min=.5,R_star_max=2.,M_star_min=1.,M_star_max=1.)
                r=self.gtls(*lc,verbose=False).power(**kw)
                outputs.append(dict(periods=np.asarray(np.ma.filled(r.periods,np.nan)),
                                    power=np.asarray(np.ma.filled(r.chi2,np.nan)),
                                    native=dict(period=number(r.period),score=number(r.SDE),epoch=number(r.T0))))
        elif k=='astropy':
            for lc in lcs:
        ...
        for o in outputs:
            if 'native' in o:
                o['candidate']=o['native'];o['candidate']['finite_fraction']=float(np.isfinite(o['power']).mean())
                if o['candidate']['period'] is None or o['candidate']['score'] is None:raise ValueError('Nonfinite native TLS candidate')
...
def main():
    ...
    try:
        ...
        backend.sync();start=time.perf_counter();backend.search([lcs[ids[0]]]);backend.sync()
        ...
        if a.timing:
            ...
            chunks=None
        for indices in ([ids] if a.timing else [[i] for i in ids]):
            if not a.timing:
                backend.sync();start=time.perf_counter();outputs=backend.search([lcs[i] for i in indices]);backend.sync()
                elapsed=time.perf_counter()-start
            else:elapsed=None
            for i,o in zip(indices,outputs):
                truth=meta['cases'][i];candidate=o['candidate'];found=candidate['period']
                drift=abs(found-truth['period'])/truth['period']*meta['baseline']
                recovered=bool(drift<=.5*truth['duration']) if truth['injected'] else None
                alias=bool(any(abs(found/(truth['period']*k)-1)*meta['baseline']<=.5*truth['duration'] for k in [.5,1.,2.,1/3,3])) if truth['injected'] else None
                ...
                row=dict(index=i,injected=truth['injected'],snr=truth['target_white_oracle_snr'],
                    ...
                    search_s=elapsed,output_file=path.name,output_sha256=sha(path))
