...
def main():
    ...
    assert text.count(performance_heading)==1
    batch=[t[p,'bls_pypi','batch16']['seconds_per_source']/t[p,'bls_v1','batch16']['seconds_per_source'] for p in profiles]
    cpu=[t[p,'bls_cpu','batch16']['seconds_per_source']/t[p,'bls_v1','batch16']['seconds_per_source'] for p in profiles]
    gpu=[t[p,'bls_gpu','batch16']['seconds_per_source']/t[p,'bls_v1','batch16']['seconds_per_source'] for p in profiles]
    lead=(f'**Faster transit searches for TESS and ZTF.** On the tested cadences, v1 BLS is **{min(batch):.1f}–{max(batch):.1f}× faster than PyPI 0.2.5** '
          f'in batches, and **{min(fresh):.1f}–{max(fresh):.1f}× faster** when each source needs a new period grid. '
          'The figure pairs execution time with independent recovery tests.\n\n')
    figure=('![Transit-search speed, recovery and projected cost on TESS and ZTF cadences](docs/figures/transit_benchmarks_20260908.png)\n\n'
            'Single-source latency and batch throughput on observed cadences with synthetic transits and noise. '
            '[Results, sensitivity qualifications and methodology](docs/TRANSIT_BENCHMARKS.md) · '
            '[PDF figure](docs/figures/transit_benchmarks_20260908.pdf)\n\n')
    production=next(line for line in lines if line.startswith('cuvarbase is built for processing millions'))
    other_results=[line for line in lines if line.startswith(('- **Survey-scale Lomb-Scargle','- **Keplerian frequency grids'))]
    story=[performance_heading.rstrip(),'',production,'',
        ...
        f'Against external BLS implementations, measured batch searches were **{min(cpu):.0f}–{max(cpu):.0f}× faster than the strongest tested CPU settings** '
        f'(Astropy or periodfind) and **{min(gpu):.1f}–{max(gpu):.1f}× faster than periodfind GPU**. The figure marks the comparisons whose recovery and false-positive results support the stated 5-point criterion.','',
        ...
        f'The resulting v1 TLS batch searches were **{min(tls):.0f}–{max(tls):.0f}× faster than public GTLS**, but the two implementations use different numerical searches. '
        '**Equivalent TLS detection sensitivity is not established by this experiment.** On ZTF, v1 recovered more transits and also accepted more nulls. The timing advantage is measured; its recovery tradeoff remains part of the comparison.','',
        ...
        'Earlier benchmarks cover other performance features:','',*other_results,'',
    ...
    start=text.index(performance_heading);end=text.index('## Features\n',start)
    p.write_text(text[:start]+lead+figure+'\n'.join(story)+'\n'+text[end:])
