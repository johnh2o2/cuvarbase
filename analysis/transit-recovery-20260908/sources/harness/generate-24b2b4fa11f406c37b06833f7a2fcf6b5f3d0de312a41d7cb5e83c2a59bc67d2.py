...
def main():
    ...
    root=ap.parse_args().root;src=root/'sources';out=root/'inputs';out.mkdir(exist_ok=True)
    ...
    p=Path('analysis/release-benchmarks-20260907/inputs/real_ztf_heldout.npz')
    with np.load(p) as d:
    ...
    for pi,(name,raw) in enumerate(cadences.items()):
        ...
        for split,ninj,nnull in [('tune',32,32),('heldout',128,128)]:
            arrays=dict(freqs=frequencies,q=q,tls_periods=tls_periods)
            rows=[]
            for i in range(ninj+nnull):
                seed=820260908+100000*pi+(10000 if split=='heldout' else 0)+i
