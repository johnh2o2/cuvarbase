...
def main():
    ...
    for job in jobs:
        ...
        script=job.get('script','worker.py')
        cmd=[str(python),str(ROOT/'recovery/scripts'/script),'--input',str(ROOT/'recovery/inputs'/job['input']),
             '--config',json.dumps(cfg),'--out',str(out)]
        if script=='worker.py':cmd+=['--indices',job.get('indices','all')]
        if 'variant' in job:cmd+=['--variant',job['variant']]
