...
@pytest.fixture
def data(seed=100, sigma=0.1, ybar=12., snr=10, ndata=500, freq=10., q=0.01, phi0=None):
...
def test_bls_parameter_recovery(seed=100):
    ...
    for test in range(ntests):
        ...
        print(outstr.format(test=test, ntests=ntests, freq=freq,q=q,phi0=phi0,alpha=alpha))
        t, y, err = data(snr=10, q=q,phi0=phi0, freq=freq)
        freqs = np.linspace(freq-0.5,freq+0.5,1000)
        df = freqs[1] - freqs[0]

        power, sols = eebls_gpu(t, y, err, freqs, qmin=0.001, qmax=0.3, alpha=alpha)
