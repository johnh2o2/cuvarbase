...
class GPUBatchProcess(object):
	def __init__(self, function, device=0, reader=None, nstreams=None, function_kwargs=None):
	...
	def run(self, data):
		if len(self.streams) == 0 or (self._adjust_nstreams and len(self.streams) < len(data)):
			self._create_streams(len(data) - len(self.streams))
		print("LAUNCHING RES")
		results = self.function(self.streams, data, **self.function_kwargs)
...
def pdm_batch_function(streams, data, ndata = None, freqs=None, max_freq=0.2,oversampling_factor=1, dphi=0.05, block_size=128):
	print("    IN PDM")
	gpu_var_utils = SourceModule(open('var_inds.cu', 'r').read(), options=['--use_fast_math'])
	gpu_pdm = gpu_var_utils.get_function('pdm').prepare([ np.intp, np.intp, np.intp, np.intp, np.intp, np.int32, np.int32, np.float32, np.float32 ])
	gpu_pdm_gauss = gpu_var_utils.get_function('pdm_gauss').prepare([ np.intp, np.intp, np.intp, np.intp, np.intp, np.int32, np.int32, np.float32, np.float32 ])

	print("     COMPILED A BUNCH O SHIT")
	nsamples = len(data)

	formatted_data = [ tuple([ np.array(arr, dtype=np.float32) for arr in zip(*d) ]) for d in data ]
	print("       FORMTTED DATA")	
	if freqs is None:
		max_baseline = max([ max(t) - min(t) for t, y, err in formatted_data ])
		df = oversampling_factor / max_baseline
		Nf = int(np.ceil(max_freq / df))
		freqs = df * (1 + np.arange(Nf))
	...
	power_cpu = [ cuda.aligned_zeros(shape=(len(freqs),), 
                                         dtype=np.float32, 
                                         alignment=resource.getpagesize()) \
                                      for i in range(nsamples) ]
	print(" MADE A BUNCH OF ALIGNED ZEROS")
	power_cpu = [ cuda.register_host_memory(pcpu) for pcpu in power_cpu ]
	print(" REGUSTERED")
	print(formatted_data[0])
	x_gs = [ gpuarray.zeros(len(t), dtype=np.float32) for t, y, err in formatted_data ]
	w_gs = [ gpuarray.zeros(len(t), dtype=np.float32) for t, y, err in formatted_data ]
	y_gs = [ gpuarray.zeros(len(t), dtype=np.float32) for t, y, err in formatted_data ]
	power_gs = [ cuda.mem_alloc(pcpu.nbytes) for pcpu in power_cpu ]

	print("  MADE A BUNCH OF XYWP")
	freqs_g = gpuarray.to_gpu(freqs.astype(np.float32))
	
	print("           about to start the fun")
	for i, (times, y, errs) in enumerate(formatted_data):
		ndata = np.int32(len(times))
		...
		w_gs[i].set_async(weights, stream=stream)
		x_gs[i].set_async(times, stream=stream)
		y_gs[i].set_async(y, stream=stream)

		gpu_pdm.prepared_async_call(grid, block, stream,
				x_gs[i].ptr, y_gs[i].ptr, w_gs[i].ptr, freqs_g.ptr, power_gs[i],
				ndata, nfreqs, dphi, var)
	
		cuda.memcpy_dtoh_async(power_cpu[i], power_gs[i], stream)

	#for stream in streams:
	#	stream.synchronize()
	
	#plt.plot(freqs, power_cpu[i])	
	#plt.show()
	#sys.exit()
